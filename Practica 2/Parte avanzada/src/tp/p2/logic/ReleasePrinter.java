...
public class ReleasePrinter extends BoardPrinter implements GamePrinter{
	...
	public String encodeGame(Game game) {		
		...
		for(int i=0; i<game.getFilas(); i++) 
		{
			...
			{	
				Sun sun = game.getSunPosition(i, j);
				if(game.hayAlgo(i,j)) {					
					if(sun != null) {
						builder.append(game.toStringGameRelease(i, j)).append(" ").append(sun.getLetra()).append(this.vDelimiter);
					}
					else
						builder.append( MyStringUtils.centre(game.toStringGameRelease(i, j), this.cellSize)).append(this.vDelimiter);
				}
				else if(sun != null) {
					builder.append( MyStringUtils.centre(sun.getLetra(), this.cellSize)).append(this.vDelimiter);
				}
				else
